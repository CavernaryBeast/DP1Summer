...
public interface SubmissionRepository extends JpaRepository<Submission, Integer> {
	...
	@Query("select sub from Submission sub join sub.reviewers revs where revs.id = ?1 AND sub.notified = 0 AND sub not in (select sub1 from Report r join r.submission sub1 where r.reviewer.id = ?1)")
