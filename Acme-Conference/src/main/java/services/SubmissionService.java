...
public class SubmissionService {
	...
	public Submission assignSubmission(final int submissionId) {
		...
		final int cantidadPorA�adir = 3 - submission.getReviewers().size();

		if (cantidadPorA�adir > 0)
			if (matchesSorted.size() > cantidadPorA�adir)
				for (int i = 0; i < cantidadPorA�adir; i++) {
		...
		Assert.isTrue(submission.getReviewers().size() <= 3, "M�ximo 3 tras el proceso");
		this.save2(submission);
		return submission;
	}
