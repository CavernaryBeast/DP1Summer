...
<spring:message code="conference.notification5Days"
	var="notificationLabel" />
<spring:message code="conference.cameraReady5Days"
	var="cameraReadyLabel" />
<spring:message code="conference.organised5Days" var="organisedLabel" />

<form:form action="${actionFilter}"
	modelAttribute="administratorfilterConferenceForm">
...
<display:table pagesize="10" name="${conferences}" id="row"
	...
	<display:column property="submissionDeadline"
		titleKey="conference.submissionDeadline" />
	<display:column property="notificationDeadline"
		titleKey="conference.notificationDeadline" />
	<display:column property="cameraReadyDeadline"
		titleKey="conference.cameraReadyDeadline" />
	...
	<security:authorize access="hasRole('ADMINISTRATOR')">
		...
		<display:column titleKey="conference.submission.update">
			<jstl:choose>
				<jstl:when
					test="${fn:contains(conferencesWithUnderReviewSubmissions, row) and now > row.submissionDeadline and now < row.cameraReadyDeadline}">
		...
		<display:column titleKey="conference.submissionList">
			<a
				href="conference/administrator/listSubmissions.do?conferenceId=${row.id}"><spring:message
		...
		<display:column titleKey="conference.reckons">
			<a href="reckon/administrator/listReckons.do?conferenceId=${row.id}"><spring:message
					code="conference.reckons" /></a>
		</display:column>
...
<security:authorize access="hasRole('ADMINISTRATOR')">
	...
	<input type="button" name="create"
		value="<spring:message code = 'conference.create' />" class="btn"
