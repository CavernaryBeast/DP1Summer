...
<p>
	<spring:message code="submission.ticker" />
	:
...
<p>
	<spring:message code="submission.author" />
	:
	<jstl:out value="${submission.author.userAccount.username}" />
</p>
...
<fieldset>
	...
	<p>
		<spring:message code="submission.paper.showAuthors" />
		:
	</p>
	...
	<p>
		<spring:message code="submission.paper.cameraReady" />
		:
		<jstl:out value="${submission.paper.cameraReady}" />
	</p>

	<jstl:if test="${submission.notified eq true}">
		<display:table name="${reports}" class="displaytagg" pagesize="10"
			id="row">
			<display:column titleKey="submission.reports">
				<a href="report/author/display.do?reportId=${row.id}"><spring:message
